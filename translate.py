...
class EncoderRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        word_embeddings = self.embedding(input).cuda()
        outputs, hn = self.rnn.forward(word_embeddings)
        return outputs, hn
...
class DecoderRNN(nn.Module):
    ...
    def __init__(self, no_of_output_symbols, embedding_size=16, hidden_size=25, use_attention=True,
        ...
        self.output = nn.Linear(hidden_size, no_of_output_symbols).cuda()
        self.device = device
        self.to(device)

    def forward(self, inp, hidden, encoder_outputs):
        """
        input is a list of length batch_size
        ...
        if self.use_attention:
            l, B, T = hidden.size()
            input = torch.tensor(inp).cuda()
            emb = self.embedding(input)
            B, T = emb.size()
            emb = emb.reshape(B, 1, T)

            wh = torch.matmul(encoder_outputs, self.W)  # 40 21 200
            us = torch.transpose(torch.matmul(hidden, self.U), 0, 1)  # 40 1 200
            e = torch.sigmoid(wh + us)
            e = torch.matmul(e, self.v)
            att = torch.softmax(e, dim=1)
            context = torch.sum(att * encoder_outputs, dim=1).unsqueeze(0)
            out, hn = self.rnn.forward(emb, context)
            out = self.output(out)
            return out, hn, att
            ...
            B, T = emb.size()
            emb = emb.reshape(B, 1, T)
            out, hn = self.rnn.forward(emb, hidden)
            out = self.output(out)
...
def evaluate(ds, encoder, decoder):
    ...
    for x, y in ds:
        ...
        for correct in y:
            if use_attention:
                predictions, hidden, _ = decoder([predicted_symbol], hidden, outputs)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tr', '--train', default='eng-swe-train.txt', help='A training file')
    parser.add_argument('-de', '--dev', default='eng-swe-dev.txt', help='A test file')
    parser.add_argument('-te', '--test', default='eng-swe-test.txt', help='A test file')
    ...
    if args.load:
        ...
        for epoch in range(args.epochs):
            total_loss = 0
            for source, target in tqdm(training_loader, desc="Epoch {}".format(epoch + 1)):
                ...
                for i in range(target_length):
                    ...
                    if use_attention:
                        if use_teacher_forcing:
                            predictions, hidden, _ = decoder(idx, hidden, outputs)
                        else:
